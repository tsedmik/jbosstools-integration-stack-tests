...
public class DataTransformationDeploymentTest extends DefaultTest {
	 ...
	 */
	@Before
	public void setupPrepareProject() {
		try {
			FileUtils.copyDirectory(ResourceHelper.getResourceAbsolutePath(Activator.PLUGIN_ID, "resources/projects/XML-to-JSON"),
					ResourceHelper.getResourceAbsolutePath(Activator.PLUGIN_ID, "target/XML-to-JSON"));
		} catch (IOException e1) {
			e1.printStackTrace();
			fail("Cannot copy a Fuse project into 'target' folder!");
		}
	}
	...
	public void testDeployment() {
		...
		new ImportMavenWizard().importProject(ResourceHelper.getResourceAbsolutePath(Activator.PLUGIN_ID, "target/XML-to-JSON"));
		CamelProject project = new CamelProject("camel-blueprint-transformation-test");
		project.close();
		project.open();
		project.enableCamelNature();
		ServerManipulator.addModule(serverRequirement.getConfig().getName(), "camel-blueprint-transformation-test");
